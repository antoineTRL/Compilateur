...
static void buf_mod_backwards(size_t *val, size_t tosub) {
    *val = (*val - tosub + BUF_SIZE) % BUF_SIZE;
}
...
void buf_print(buffer_t *buffer) {
    ...
    for (size_t i = 0; i < BUF_SIZE; i++) {
        ...
        if (buffer->it == i)
            color = "\033[1;32m"; // Green background
        else if (buffer->end == i)
            color = "\033[1;31m"; // Red background
        else if (buffer->islocked && buffer->lock == i)
            color = "\033[1;34m"; // Blue background
        if (buffer->content[i] == '\n')
            printf("%s %s\n", color, "\033[0m");
        else
            printf("%s%c\033[0m", color, buffer->content[i]);
    }
    printf("#### </buffer> ####\n");
}
